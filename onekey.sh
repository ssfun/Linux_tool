...
install_plex() {
    ...
    cd `mktemp -d`
    curl -sSL "${PLEX_LINK}" -O plexmediaserver.deb
...
download_fb() {
    ...
    cd `mktemp -d`
    curl -sSL "${FILEBROWSER_LINK}" -O filebrowser.tar.gz
...
download_sing-box() {
    ...
    cd `mktemp -d`
    curl -sSL "${LINK}" -O sing-box.tar.gz
...
download_caddy() {
    ...
    cd `mktemp -d`
    curl -sSL "${CADDY_LINK}" -O caddy.tar.gz
