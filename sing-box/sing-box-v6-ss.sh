...
download_sing-box() {
    ...
    LATEST_VERSION="$(wget -qO- -t1 -T2 "https://proxy.sfun.eu.org/sfun/https/api.github.com/repos/SagerNet/sing-box/releases" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')"
    LATEST_NUM="$(wget -qO- -t1 -T2 "https://proxy.sfun.eu.org/sfun/https/api.github.com/repos/SagerNet/sing-box/releases" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/v//g;s/,//g;s/ //g')"
    LINK="https://proxy.sfun.eu.org/sfun/https/github.com/SagerNet/sing-box/releases/download/${LATEST_VERSION}/sing-box-${LATEST_NUM}-linux-${ARCH}.tar.gz"
    cd `mktemp -d`
    ...
    cat <<EOF >${SING_BOX_CONFIG_PATH}/config.json
{
  ...
  "route": {
    ...
    "geoip": {
      "path": "geoip.db",
      "download_url": "https://proxy.sfun.eu.org/sfun/https/github.com/SagerNet/sing-geoip/releases/latest/download/geoip.db",
      "download_detour": "direct"
    },
    "geosite": {
      "path": "geosite.db",
      "download_url": "https://proxy.sfun.eu.org/sfun/https/github.com/SagerNet/sing-geosite/releases/latest/download/geosite.db",
      "download_detour": "direct"
    },
    "final": "proxy_ipv6",
